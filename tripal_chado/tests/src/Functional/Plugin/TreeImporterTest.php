...
/**
 * Tests for the TreeImporter class
 *
 * @group Tripal
 * @group Tripal Chado
 * @group Tripal Chado ChadoStorage
 * @group Tripal Chado ChadoStorage TreeImporter
 */
class TreeImporterTest extends ChadoTestBrowserBase
{
  /**
   * Confirm basic Tree importer functionality.
   *
   * @group taxonomy
   */
  public function testTreeImporterSimpleTest()
  {
    ...
    // Create a Tree Importer
    // Perform the Tree Importer test by creating an instance of the Tree loader
    $importer_manager = \Drupal::service('tripal.importer');
    $tree_importer = $importer_manager->createInstance('chado_tree_loader');
    $run_args = [
      'schema_name' => $schema_name,
      'tree_name' => 'Taxonomy Tree',
      'analysis_name' => 'Taxonomy Tree',
      'use_transaction' => 1,
    ];

    $file_details = [
      'file_local' => __DIR__ . '/../../../fixtures/tree_loader/newick_T92076.tree',
    ];

    $tree_importer->create($run_args, $file_details);
    $tree_importer->prepareFiles();
    $tree_importer->run();
    $tree_importer->postRun();
