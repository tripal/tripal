...
/**
 * Tests the ChadoRealTypeDefault field type.
 *
 * Covers create, load, and update operations through TripalEntity,
 * ChadoStorage, and the field plugin layer. Uses
 * chado.phylonode.distance as the backing column since it is
 * the simplest Chado table with a real (double precision) column.
 *
 * @group tripal-field
 * @group chado-field
 */
...
class ChadoRealTypeCRUDTest extends ChadoTestKernelBase {
   ...
   * @var string
   */
  protected string $yaml_info_file = __DIR__ . '/ChadoRealType-TestInfo.yml';
  ...
  public static function provideScenarios(): array {
    return [
      [0, 'Create and load a latitude value'],
    ];
  }

  /**
   * Tests the ChadoRealTypeDefault field through TripalEntity->save().
   *
   * @dataProvider provideScenarios
   */
  #[DataProvider('provideScenarios')]
  public function testChadoRealTypeEntityCrud(int $current_scenario_key, string $current_scenario_label): void {
