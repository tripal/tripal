...
class ChadoStockBuddyTest extends ChadoTestBuddyBase {
  ...
  */
  ...
  public function testRelateStock() {
    ...
    try {
      $status = $stock_instance->relateStock(['buddy_record' => $subject_stock_record], ['buddy_record' => $object_stock_record], ['buddy_record' => $cvterm_record]);
      $this->fail("We did not trigger an exception when relating the same two stock records a fourth time without specifying a unique rank.");
    }
    catch (\Exception $e) {
  ...
  public static function provideStockBuddyExceptionScenarios() {
    ...
    // #1: insertStock() with no organism values.
    $scenarios[] = [
      'insertStock',
      [
        [
      ...
      "ChadoBuddy insertStock error, a unique organism record must be provided to insert a stock record.",
    ];

    // #2: insertStock() with no stock type values.
    $scenarios[] = [
      'insertStock',
      [
        [
          'stock.name' => 'StockWithNoType',
          'organism.genus' => 'Tripalus',
          'organism.species' => 'databasica',
        ],
      ],
    ...
    // #3: insertStock() where stock.dbxref_id and dbxref.dbxref_id don't match.
    $scenarios[] = [
      'insertStock',
      [
        [
    ...
    // #4: insertStock() with dbxref values that match more than one dbxref_id.
    $scenarios[] = [
      'insertStock',
      [
        [
    ...
    // #5: insertStock() where dbxref values are provided but could not find a
    ...
    $scenarios[] = [
      ...
      [
        [
          'stock.name' => 'StockWithDbxrefButNoType',
          'stock.uniquename' => 'stockwithdbxrefbutnotype',
          'organism.genus' => 'Tripalus',
          'organism.species' => 'databasica',
          'db.name' => 'EFO',
          'dbxref.accession' => '0005136',
