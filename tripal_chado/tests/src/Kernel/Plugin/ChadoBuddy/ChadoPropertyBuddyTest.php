...
class ChadoPropertyBuddyTest extends ChadoTestBuddyBase {
   ...
   *
   * Tests the getProperty(), insertProperty(), updateProperty(),
   * and deleteProperty() methods.
   */
  public function testPropertyMethods() {
    ...
    $test_records = [];
    $test_records['set'] = $instance->insertProperty('project', 1,
      [
        'projectprop.type_id' => 1,
        'projectprop.value' => 'prop001',
      ], []);
    ...
    $test_records = [];
    $test_records['set'] = $instance->updateProperty('project', 1,
      [
        'projectprop.type_id' => 2,
        'projectprop.value' => 'prop002',
        'projectprop.rank' => 5,
      ],
    ...
    $chado_buddy_record = $instance->updateProperty('project', 1,
      [
        'projectprop.type_id' => 2,
        'projectprop.value' => 'prop000',
        'projectprop.rank' => 5,
      ],
    ...
    $test_records = [];
    $values_3 = ['projectprop.type_id' => 1, 'projectprop.value' => 'prop003', 'projectprop.rank' => 5];
    $test_records['set'] = $instance->upsertProperty('project', 1, $values_3, []);
    $test_records['get'] = $instance->getProperty('project', 1, $values_3, []);
    ...
    $exception_message = '';
    try {
      $chado_buddy_record = $instance->insertProperty('project', 1,
        [
          'projectprop.type_id' => 1,
          'projectprop.value' => 'prop001',
          'projectprop.rank' => 5,
    ...
    // TEST: Inserting a property without specifying a Cvterm should
    // throw an exception.
    $exception_caught = FALSE;
    $exception_message = '';
    try {
    ...
    // Value is not in a unique constraint, so will be updated.
    $test_records = [];
    $test_records['set'] = $instance->upsertProperty('project', 1,
      [
        'projectprop.type_id' => 1,
        'projectprop.value' => 'prop004',
        'projectprop.rank' => 5,
      ], []);
    $test_records['get'] = $instance->getProperty('project', 1,
      [
        'projectprop.type_id' => 1,
        'projectprop.rank' => 5,
      ], []);
    ...
    // TEST: Update should throw an exception if it matches
    ...
    // TEST: Upsert should throw an exception if it matches more
    // than one record.
    $chado_buddy_records = $instance->getProperty('project', 1, ['projectprop.rank' => 5], []);
    ...
    $exception_message = '';
    try {
      $chado_buddy_record = $instance->upsertProperty('project', 1,
        [
          'projectprop.value' => 'propfail',
          'projectprop.rank' => 5,
        ], []);
    ...
    // TEST: We should be able to insert a property record even if the cvterm
    // doesn't exist. This tests a use case for an importer, both term and
    // dbxref are automatically created.
    // This needs the opt-in flag 'create_cvterm'.
    $test_records = [];
    $test_records['set'] = $instance->insertProperty('project', 1,
      [
        'projectprop.value' => 'prop005',
        'projectprop.rank' => 5,
        'db.name' => 'local',
        'cv.name' => 'local',
        'cvterm.name' => 'name005',
        'dbxref.accession' => 'acc005',
      ],
      ['create_cvterm' => TRUE]
    );
    $test_records['get'] = $instance->getProperty('project', 1,
    ...
    // TEST: we can pass cvterm.cvterm_id instead of projectprop.type_id
    // for an upsert.
    $test_records = [];
    $values_6 = ['cvterm.cvterm_id' => 2, 'projectprop.value' => 'prop006'];
    $test_records['set'] = $instance->upsertProperty('project', 1, $values_6, []);
    $test_records['get'] = $instance->getProperty('project', 1, $values_6);
