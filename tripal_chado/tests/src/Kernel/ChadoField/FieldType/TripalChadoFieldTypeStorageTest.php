...
class TripalChadoFieldTypeStorageTest extends ChadoTestKernelBase {
   ...
   * @var string
   */
  protected string $yaml_info_file = __DIR__ . '/TripalChadoFieldTypeStorageTest-TestInfo.yml';
   ...
   */
  public function testChadoFieldLocalCaching() {
    ...
    foreach ($this->system_under_test['fields'] as $fieldyml) {
      ...
      foreach ($field_items as $delta => $field_item) {
        ...
        $this->assertCount(2, $prop_types, "We did not get the expected number of property TYPES when trying to retrieve them ALL.");
        ...
        $this->assertCount(2, $prop_values, "We did not get the expected number of property VALUES when trying to retrieve them ALL.");
    ...
    $field_item = $entity->get('organism_genus')[0];
    // Property Type.
    ...
    $this->assertEquals(
      "Cannot access the 'NON-EXISTING-PROPERTY' property type for 'organism_genus' field as it is not defined by its Drupal\\tripal_chado\Plugin\Field\FieldType\ChadoStringTypeDefault::tripalTypes method.",
      $exception_msg,
    ...
    $this->assertEquals(
      "Cannot access the 'NON-EXISTING-PROPERTY' property value for 'organism_genus' field.",
      $exception_msg,
      "We did not get the exception message we expected.",
    );
  }
   ...
   */
  public function testChadoFieldPropertyFieldSync() {
  ...
  public function testTripalEntityFieldStorageInteractions() {
    ...
    // For the organism_genus field, lets confirm we can access the information
    // that is supported.
    $this->assertEquals('value', $entity->getTripalFieldInfo('organism_genus', 'main_property'), "We were not able to retrieve the main property name for the organism_genus field.");
    // $this->assertEquals(TRUE, $entity->getTripalFieldInfo('organism_genus', 'fully_cached'), "We were not able to retrieve the Tripal cache status of the organism_genus field.");
    // Confirm that the field knows if it is empty or not.
    $organism_genus = [
      'items' => $entity->get('organism_genus'),
    ];
    $organism_genus['prop_values'] = $organism_genus['items']->first()->getTripalStoragePropertyValues();
    $organism_genus['store_values'] = ['value' => $organism_genus['items']->first()->getValue()];
    // A) the organism_genus field should not be empty.
    $is_empty = TripalEntity::isEmptyFieldItem('organism_genus', $organism_genus['items'], $organism_genus['prop_values'], $organism_genus['store_values']);
    $this->assertFalse($is_empty, "The organism_genus field should not be considered empty.");
    // B) Now set the value to NULL and confirm it is empty.
    $organism_genus['items']->first()->set('value', NULL);
    $organism_genus['prop_values']['value']->setValue(NULL);
    $organism_genus['store_values']['value'] = NULL;
    $is_empty = TripalEntity::isEmptyFieldItem('organism_genus', $organism_genus['items'], $organism_genus['prop_values'], $organism_genus['store_values']);
    $this->assertTrue($is_empty, "The organism_genus should now catch the 'allNull' case of empty.");
    // C) Now set the value to empty string and confirm it is empty.
    $organism_genus['items']->first()->set('value', '');
    $organism_genus['prop_values']['value']->setValue('');
    $organism_genus['store_values']['value'] = '';
    $is_empty = TripalEntity::isEmptyFieldItem('organism_genus', $organism_genus['items'], $organism_genus['prop_values'], $organism_genus['store_values']);
    $this->assertTrue($is_empty, "The organism_genus should now catch the 'zero value in store values' case of empty.");
    ...
    $exception_msg = uniqid();
    try {
      $entity->getTripalFieldInfo('title', 'main_property');
    }
    catch (\Exception $e) {
      $exception_caught = TRUE;
      $exception_msg = $e->getMessage();
    ...
    $exception_msg = uniqid();
    try {
      $entity->getTripalFieldInfo($field_name, 'main_property');
    }
    catch (\Exception $e) {
      $exception_caught = TRUE;
      $exception_msg = $e->getMessage();
  ...
  /**
   * Tests the deprecated field tripal*() methods.
   *
   * Note: This test should match the one for testing the new methods which
   * replace these ones.
   *
   * @see ::testTripalFieldPropertyFieldSync()
   */
  #[IgnoreDeprecations]
  public function testDeprecatedFieldMethods() {
