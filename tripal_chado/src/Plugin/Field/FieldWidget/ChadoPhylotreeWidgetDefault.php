...
/**
 * Plugin implementation of default Chado phylotree widget.
 */
#[TripalFieldWidget(
  id: 'chado_phylotree_widget_default',
  label: new TranslatableMarkup('Chado Phylotree Widget'),
  description: new TranslatableMarkup('The default phylotree widget.'),
  field_types: [
    'chado_phylotree_type_default',
  ],
)]
class ChadoPhylotreeWidgetDefault extends ChadoWidgetBase {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    $linker_fkey_column = $storage_settings['linker_fkey_column']
      ?? $storage_settings['base_column'] ?? 'phylotree_id';

    // Reverse lookups can only be added on the corresponding content type.
    if ($linker_fkey_column == "\u{2605}reverse_link") {
      return [
        'disabled' => [
          '#markup' => $this->t('Note: Links between this content and a phylotree can only be edited on a phylotree content type.'),
        ],
      ];
    }
    ...
    $phylotree_id = $item_vals[$linker_fkey_column] ?? 0;
    ...
    $options = [
      ...
      'column_name' => 'name',
      'type_column' => 'x',
      'property_table' => 'phylotree',
    ];
    $select_element = $this->genericSelectElement('phylotree_id', $phylotree_id, $options);
  ...
  public function massageFormValues(array $values, array $form, FormStateInterface $form_state) {
    $values = $this->genericSelectMassageFormValues('phylotree_id', $values);
    return $this->massageLinkingFormValues('phylotree_id', $values, $form_state);
  }
