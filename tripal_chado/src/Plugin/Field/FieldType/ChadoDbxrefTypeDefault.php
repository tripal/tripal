...
class ChadoDbxrefTypeDefault extends ChadoFieldItemBase {
  ...
  public static function tripalTypes($field_definition) {
    ...
    $db_term = $mapping ? $mapping->getColumnTermId($object_table, 'db_id') : 'ERO:0001716';
    $accession_term = $mapping ? $mapping->getColumnTermId($object_table, 'accession') : 'data:2091';
    $accession_len = $object_schema_def['fields']['accession']['size'];
    $version_term = $mapping ? $mapping->getColumnTermId($object_table, 'version') : 'IAO:0000129';
    $version_len = $object_schema_def['fields']['version']['size'];
    $description_term = $mapping ? $mapping->getColumnTermId($object_table, 'description') : 'schema:description';  // text

    // Columns from linked tables
    $db_schema_def = $schema->getTableDef('db', ['format' => 'Drupal']);
    $db_name_term = $mapping ? $mapping->getColumnTermId('db', 'name') : 'ERO:0001716';
    $db_name_len = $db_schema_def['fields']['name']['size'];
    $db_description_term = $mapping ? $mapping->getColumnTermId('db', 'description') : 'schema:description';
    $db_description_len = $db_schema_def['fields']['description']['size'];
    $db_urlprefix_term = $mapping ? $mapping->getColumnTermId('db', 'urlprefix') : 'schema:ItemPage';
    $db_urlprefix_len = $db_schema_def['fields']['urlprefix']['size'];
    $db_url_term = $mapping ? $mapping->getColumnTermId('db', 'url') : 'schema:url';
    ...
    if ($linker_table != $base_table) {
      ...
      $linker_left_term = $mapping ? $mapping->getColumnTermId($linker_table, $linker_left_col) : self::$record_id_term;
      $linker_fkey_term = $mapping ? $mapping->getColumnTermId($linker_table, $linker_fkey_column) : self::$record_id_term;
      ...
      foreach (array_keys($linker_schema_def['fields']) as $column) {
        if (($column != $linker_pkey_col) and ($column != $linker_left_col) and ($column != $linker_fkey_column)) {
          $term = $mapping ? $mapping->getColumnTermId($linker_table, $column) : 'NCIT:C25712';
          if ($term) {
      ...
      $linker_fkey_term = $mapping ? $mapping->getColumnTermId($base_table, $linker_fkey_column) : self::$record_id_term;
    }
