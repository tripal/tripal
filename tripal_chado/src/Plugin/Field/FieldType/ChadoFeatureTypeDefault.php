...
class ChadoFeatureTypeDefault extends ChadoFieldItemBase {
  ...
  public static function tripalTypes($field_definition) {
    ...
    $name_term = this::getColumnTermId($object_table, 'name', 'schema:name');
    $name_len = $object_schema_def['fields']['name']['size'];
    $uniquename_term = this::getColumnTermId($object_table, 'uniquename', 'data:0842'); // text
    // residues is not implemented in this field since it can be millions of characters long
    $seqlen_term = this::getColumnTermId($object_table, 'seqlen', 'data:1249');
    $md5checksum_term = this::getColumnTermId($object_table, 'md5checksum', 'data:2190');
    $md5checksum_len = $object_schema_def['fields']['md5checksum']['size'];
    $is_analysis_term = this::getColumnTermId($object_table, 'is_analysis', 'local:is_analysis'); // boolean
    $is_obsolete_term = this::getColumnTermId($object_table, 'is_obsolete', 'local:is_obsolete'); // boolean
    ...
    $db_term = this::getColumnTermId('db', 'name', 'ERO:0001716');
    $cvterm_schema_def = $schema->getTableDef('cvterm', ['format' => 'Drupal']);
    $type_term = this::getColumnTermId('cvterm', 'name', 'schema:additionalType');
    $type_len = $cvterm_schema_def['fields']['name']['size'];
    $organism_schema_def = $schema->getTableDef('organism', ['format' => 'Drupal']);
    $genus_term = this::getColumnTermId('organism', 'genus', 'TAXRANK:0000005');
    $genus_len = $organism_schema_def['fields']['genus']['size'];
    $species_term = this::getColumnTermId('organism', 'species', 'TAXRANK:0000006');
    $species_len = $organism_schema_def['fields']['species']['size'];
    $infraspecific_name_term = this::getColumnTermId('organism', 'infraspecific_name', 'TAXRANK:0000045');
    $infraspecific_name_len = $organism_schema_def['fields']['infraspecific_name']['size'];
    $abbreviation_term = this::getColumnTermId('organism', 'abbreviation', 'local:abbreviation');
    $abbreviation_len = $organism_schema_def['fields']['abbreviation']['size'];
    $common_name_term = this::getColumnTermId('organism', 'common_name', 'TAXRANK:0000045');
