...
//----------------------+-----------------------------+-----------+----------+-----------------------------------------
// assay_id             | bigint                      |           | not null | nextval('assay_assay_id_seq'::regclass)
// arraydesign_id       | bigint                      |           | not null | 
// protocol_id          | bigint                      |           |          | 
// assaydate            | timestamp without time zone |           |          | CURRENT_TIMESTAMP
...
// description          | text                        |           |          | 
    ...
    $properties[] = new ChadoTextStoragePropertyType($entity_type_id, self::$id, 'assay_operator', $operator_term, [
      'action' => 'read_value',
      'drupal_store' => FALSE,
      'path' => $linker_table . '.' . $linker_fkey_col . '>' . $object_table . '.' . $object_pkey_col
