...
class ChadoArrayDesignTypeDefault extends ChadoFieldItemBase {
  ...
  public static function tripalTypes($field_definition) {
    ...
    $name_term = this::getColumnTermId($object_table, 'name', 'schema:name');  // text
    $description_term = this::getColumnTermId($object_table, 'description', 'schema:description');  // text
    $version_term = this::getColumnTermId($object_table, 'version', 'IAO:0000129');  // text
    $array_dimensions_term = this::getColumnTermId($object_table, 'array_dimensions', 'local:array_dimensions');  // text
    $element_dimensions_term = this::getColumnTermId($object_table, 'element_dimensions', 'local:element_dimensions');  // text
    $num_of_elements_term = this::getColumnTermId($object_table, 'num_of_elements', 'local:num_of_elements');
    $num_array_rows_term = this::getColumnTermId($object_table, 'num_array_rows', 'local:num_array_rows');
    $num_array_columns_term = this::getColumnTermId($object_table, 'num_array_columns', 'local:num_array_columns');
    $num_grid_columns_term = this::getColumnTermId($object_table, 'num_grid_columns', 'local:num_grid_columns');
    $num_grid_rows_term = this::getColumnTermId($object_table, 'num_grid_rows', 'local:num_grid_rows');
    $num_sub_columns_term = this::getColumnTermId($object_table, 'num_sub_columns', 'local:num_sub_columns');
    $num_sub_rows_term = this::getColumnTermId($object_table, 'num_sub_rows', 'local:num_sub_rows');

    // Columns from linked tables
    // both platformtype and substratetype reference the cvterm table
    $cvterm_schema_def = $schema->getTableDef('cvterm', ['format' => 'Drupal']);
    $type_term = this::getColumnTermId('cvterm', 'name', 'rdfs:type');
    $type_len = $cvterm_schema_def['fields']['name']['size'];
    $contact_schema_def = $schema->getTableDef('contact', ['format' => 'Drupal']);
    $manufacturer_term = this::getColumnTermId('contact', 'name', 'EFO:0001728');
    $manufacturer_len = $contact_schema_def['fields']['name']['size'];
    $protocol_term = this::getColumnTermId('protocol', 'name', 'sep:00101');  // text
    $dbxref_term = this::getColumnTermId('dbxref', 'accession', 'data:2091');
    $db_schema_def = $schema->getTableDef('db', ['format' => 'Drupal']);
    $db_term = this::getColumnTermId('db', 'name', 'schema:name');
    ...
    if ($linker_table != $base_table) {
      ...
      $linker_left_term = this::getColumnTermId($linker_table, $linker_left_col, self::$record_id_term);
      $linker_fkey_term = this::getColumnTermId($linker_table, $linker_fkey_column, self::$record_id_term);
      ...
      foreach (array_keys($linker_schema_def['fields']) as $column) {
        if (($column != $linker_pkey_col) and ($column != $linker_left_col) and ($column != $linker_fkey_column)) {
          $term = this::getColumnTermId($linker_table, $column, 'NCIT:C25712');
          if ($term) {
      ...
      $linker_fkey_term = this::getColumnTermId($base_table, $linker_fkey_column, self::$record_id_term);
    }
