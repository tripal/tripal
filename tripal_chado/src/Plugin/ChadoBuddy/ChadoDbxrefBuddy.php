...
class ChadoDbxrefBuddy extends ChadoBuddyPluginBase {
   ...
   *     - db.name: The name of the database. This name is usually used as the
   *       prefix for CV term accessions.
   *     - db.description: (Optional) A description of the database.
   *       By default no description is required.
  ...
  public function insertDb(array $values, array $options = []) {
    ...
    $existing_records = $this->getDb($values, $options);
   ...
   *
   * The database must already exist. If necessary first create
   * it with insertDb().
   *
   * @param array $values
   *   An associative array of the values to be inserted including:
   *     - dbxref.dbxref_id: not valid for an insert.
   *     - dbxref.db_id or db.db_id: (Required) the database_id of the
   *       database the reference is to.
   ...
   *     - db.name: may be used in place of db.db_id or dbxref.db_id if
  ...
  public function insertDbxref(array $values, array $options = []) {
    ...
    // If db.name specified, but not db.db_id or dbxref.db_id,
    // then lookup db.db_id.
    if (!array_key_exists('dbxref.db_id', $values) or !$values['dbxref.db_id']) {
      ...
      $existing_records = $this->getDb(['db.name' => $values['db.name']], $options);
      ...
      $values['dbxref.db_id'] = $existing_records[0]->getValue('db.db_id');
    ...
    // Retrieve the newly inserted record.
    $existing_records = $this->getDbxref($dbxref_values, $options);
   ...
   * to select the existing record. db.db_id in the $values array will be
   * ignored.
   *
   * @param array $values
   ...
   *     - dbxref.db_id or db.db_id: The database_id of the database the
   *       reference is to.
  ...
  public function updateDbxref(array $values, array $conditions, array $options = []) {
    ...
    $existing_records = $this->getDbxref($values, $options);
   ...
   *     - db.db_id: primary key for db table.
   *     - db.name: The name of the database. This name is usually used as
   *       the prefix for CV term accessions.
   *     - db.description: (Optional) A description of the database.
   *       By default, no description is required.
   *     - db.url: (Optional) The URL for the database.
  ...
  /**
   * Insert a database reference.
   *
   * Inserts if it doesn't yet exist, or updates it if it does.
   *
   * @param array $values
   *   An associative array of the values for the final record including:
   *     - dbxref.dbxref_id: the primary key of the dbxref table.
  ...
  /**
   * Add a record to a database reference linking table.
   *
   * For example, the feature_dbxref table.
   *
   * @param string $base_table
   *   The base table for which the dbxref should be associated. Thus to
   *   associate a dbxref with a feature the basetable=feature and
   *   dbxref_id is added to the feature_dbxref table.
   * @param int $record_id
