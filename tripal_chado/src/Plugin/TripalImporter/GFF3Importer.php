...
class GFF3Importer extends ChadoImporterBase {
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    $target_organism_id = $form_values['target_organism_id'];
    $target_type = trim($form_values['target_type']);
    $create_target = $form_values['create_target'];
    $create_organism = $form_values['create_organism'];
    $refresh = 0; //$form_values['refresh'];
    $remove = 0; //$form_values['remove'];
    $line_number = trim($form_values['line_number']);
    $landmark_type = trim($form_values['landmark_type']);
    $alt_id_attr = trim($form_values['alt_id_attr']);
    $re_mrna = trim($form_values['re_mrna']);
    $re_protein = trim($form_values['re_protein']);
    ...
    if (!($re_mrna and $re_protein) and ($re_mrna or $re_protein)) {
      $form_state->setErrorByName('re_uname', t("You must provide both a regular expression for mRNA and a replacement string for protein"));
    }
    ...
    if ($result_re === FALSE) {
      ...
      if ($result === FALSE) {
        $form_state->setErrorByName('re_protein', 'Invalid replacement string.');
      }
    }
  }
  ...
  public function run() {
    ...
    // Open the GFF3 file.
    $this->logger->info("Opening " . $this->gff_file);
    ...
    $this->feature_prop_cv = new \Drupal\tripal_chado\Services\ChadoRecord('cv');
    ...
    $this->feature_cv = new \Drupal\tripal_chado\Services\ChadoRecord('cv');
    ...
    $this->organism = new \Drupal\tripal_chado\Services\ChadoRecord('organism');
    ...
    $this->analysis = new \Drupal\tripal_chado\Services\ChadoRecord('analysis');
    ...
    if ($this->landmark_type) {
      $this->landmark_cvterm = new \Drupal\tripal_chado\Services\ChadoRecord('cvterm');
    ...
    if ($this->target_type) {
      $target_type = new \Drupal\tripal_chado\Services\ChadoRecord('cvterm');
    ...
    // Load the GFF3.
    try {
      $this->logger->info("Step  1 of 27: Caching GFF3 file...                             ");
      $this->parseGFF3();
      ...
      $this->logger->info("Step  2 of 27: Find existing landmarks...                         ");
      $this->findLandmarks();

      $this->logger->info("Step  3 of 27: Insert new landmarks (if needed)...                ");
      $this->insertLandmarks();

      if (!$this->skip_protein) {
        $this->logger->info("Step  4 of 27: Find missing proteins...                           ");
        $this->findMissingProteins();

        $this->logger->info("Step  5 of 27: Add missing proteins to list of features...        ");
        $this->addMissingProteins();
      }
      else {
        $this->logger->info("Step  4 of 27: Find missing proteins (Skipped)...                ");
        $this->logger->info("Step  5 of 27: Add missing proteins to list of features (Skipped)...");
      }

      $this->logger->info("Step  6 of 27: Find existing features...                          ");
      $this->findFeatures();

      $this->logger->info("Step  7 of 27: Clear attributes of existing features...            ");
      $this->deleteFeatureData();

      $this->logger->info("Step  8 of 27: Processing " . number_format(count(array_keys($this->features))) . " features...               ");
      $this->insertFeatures();
      $this->logger->info("Step  9 of 27: Processing " . number_format(count(array_keys($this->update_names)))  . " feature Names to update...               ");
      $this->updateFeatureNames();

      $this->logger->info("Step  10 of 27: Get new feature IDs...                             ");
      $this->findFeatures();

      $this->logger->info("Step 11 of 27: Insert locations...                               ");
      $this->insertFeatureLocs();

      $this->logger->info("Step 12 of 27: Associate parents and children...                 ");
      $this->associateChildren();

      $this->logger->info("Step 13 of 27: Calculate child ranks...                          ");
      $this->calculateChildRanks();

      $this->logger->info("Step 14 of 27: Add child-parent relationships...                 ");
      $this->insertFeatureParents();

      $this->logger->info("Step 15 of 27: Insert properties...                               ");
      $this->insertFeatureProps();

      $this->logger->info("Step 16 of 27: Find synonyms (aliases)...                         ");
      $this->findSynonyms();

      $this->logger->info("Step 17 of 27: Insert new synonyms (aliases)...                  ");
      $this->insertSynonyms();

      $this->logger->info("Step 18 of 27: Insert feature synonyms (aliases)...              ");
      $this->insertFeatureSynonyms();

      $this->logger->info("Step 19 of 27: Find cross references...                          ");
      $this->findDbxrefs();

      $this->logger->info("Step 20 of 27: Insert new cross references...                    ");
      $this->insertDbxrefs();

      $this->logger->info("Step 21 of 27: Get new cross references IDs...                   ");
      $this->findDbxrefs();

      $this->logger->info("Step 22 of 27: Insert feature cross references...                ");
      $this->insertFeatureDbxrefs();

      $this->logger->info("Step 23 of 27: Insert feature ontology terms...                  ");
      $this->insertFeatureCVterms();

      $this->logger->info("Step 24 of 27: Insert 'derives_from' relationships...            ");
      $this->insertFeatureDerivesFrom();

      $this->logger->info("Step 25 of 27: Insert Targets...                                 ");
      $this->insertFeatureTargets();

      $this->logger->info("Step 26 of 27: Associate features with analysis....              ");
      $this->insertFeatureAnalysis();

      if (!empty($this->residue_index)) {
        $this->logger->info("Step 27 of 27: Adding sequences data...                        ");
        $this->insertFeatureSeqs();
      }
      $this->logger->info("Step 27 of 27: Adding sequences data (Skipped: none available)...");
    }
  ...
  private function prepNullPub(){
    ...
    if (count($result) == 0) {
      ...
      if (!$status) {
        $this->logger->info("Cannot prepare statement 'ins_pub_uniquename_typeid.");
        return 0;
      }
    ...
    foreach ($this->residue_index as $uniquename => $offset) {
      ...
      if (!(array_key_exists($uniquename, $this->features) and $this->features[$uniquename]) and
          !(array_key_exists($uniquename, $this->landmarks) and $this->landmarks[$uniquename])) {
            $this->logger->warning('Assigning Sequence: cannot find a feature with a unique name of: "' . $uniquename . '". Please ensure the sequence names in the ##FASTA section use the same name as the ID in the feature in the GFF file. ');
        $count++;
        continue;
      }
  ...
  private function findLandmark($landmark_name) {
    ...
    $landmark = new \Drupal\tripal_chado\Services\ChadoRecord('feature');
    $landmark->setValues([
  ...
  private function insertLandmark($name) {
    $feature = new \Drupal\tripal_chado\Services\ChadoRecord('feature');
    $residues = '';
    $feature->setValues([
  ...
  private function findMissingProteins() {
    ...
    // created.
    if ($this->skip_protein) {
      $this->logger->info('  Skipping creation of non-specified proteins...            ');
      return;
    }
  ...
  private function openCacheFile() {
    ...
    $this->logger->info("Opening temporary cache file: " . $this->gff_cache_file_name);
  ...
  private function closeCacheFile() {
    ...
    $this->logger->info("Removing temporary cache file: " . $this->gff_cache_file_name);
  ...
  private function findFeatures() {
    ...
    $sql = "SELECT uniquename, name, type_id, organism_id, feature_id FROM {feature} WHERE uniquename in ";
    $i = 0;
    $total = 0;
    $batch_num = 1;
    $names = [];
    foreach ($this->features as $uniquename => $info) {
      ...
      if ($i == $batch_size or $total == $num_features) {
        $names_counter = count($names);
        if ($names_counter > 0) {
          $args = [];
          $sql .= '(';
          for ($z = 0; $z < $names_counter; $z++) {
            $sql .= ":uniquename_" . $z . ',';
            $args[':uniquename_' . $z] = $names[$z];
          }
          $sql = rtrim($sql, ',');
          $sql .= ')';
          // $names = implode(', ', $names);
  ...
  private function deleteFeatureData() {
    ...
    $sql1 = "DELETE from {featureprop} WHERE feature_id IN ";
    $sql2 = "DELETE from {featureloc} WHERE feature_id IN ";
    $sql3 = "DELETE from {feature_cvterm} WHERE feature_id IN ";
    $sql4 = "DELETE from {feature_dbxref} WHERE feature_id IN ";
    $sql5 = "DELETE from {feature_synonym} WHERE feature_id IN ";
    $sql6 = "DELETE from {feature_relationship} WHERE subject_id IN ";
    $sql7 = "DELETE from {analysisfeature} WHERE feature_id IN ";
    $i = 0;
    $total = 0;
    $batch_num = 1;
    $feature_ids = [];
    foreach ($this->features as $info) {
      ...
      // If we've reached the size of the batch then let's do the insert.
      if ($i == $batch_size or $total == $num_features) {
        $feature_ids_counter = count($feature_ids);
        if ($feature_ids_counter > 0) {
          ...
          $sql6 .= '(';
          $sql7 .= '(';
          for ($z = 0; $z < $feature_ids_counter; $z++) {
            $sql1 .= ":feature_id_" . $z . ',';
            $sql2 .= ":feature_id_" . $z . ',';
            $sql3 .= ":feature_id_" . $z . ',';
            $sql4 .= ":feature_id_" . $z . ',';
            ...
            $sql6 .= ":feature_id_" . $z . ',';
            $sql7 .= ":feature_id_" . $z . ',';
            $args[':feature_id_' . $z] = $feature_ids[$z];
          }
          $sql1 = rtrim($sql1, ',');
          $sql2 = rtrim($sql2, ',');
          $sql3 = rtrim($sql3, ',');
  ...
  private function findOrganism($organism_attr, $line_num) {
    ...
    $organism = new \Drupal\tripal_chado\Services\ChadoRecord('organism');
    $organism->setValues([
      'genus' => $genus,
      'species' => $species
    ]);
