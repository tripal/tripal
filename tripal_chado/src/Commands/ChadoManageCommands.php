...
class ChadoManageCommands extends DrushCommands {
  ...
  /**
   * Migrate custom controlled vocabularies from Tripal 3.
   *
   * @command tripal-chado:migrate-vocabularies
   * @aliases trp-chado-migrate-vocab
   * @usage drush trp-chado-migrate-vocab
   *   Moves cvterms from several custom vocabularies to the local vocabluary,
   *   and removes the custom vocabulary.
   ...
   */
  public function migrateVocabularies() {

    // Transfer of CVs was done by PR 1727. This hook added by PR 20xx
    $obsolete_cvs = [
      'organism_property', 'analysis_property', 'tripal_phylogeny',
      'featuremap_units', 'featurepos_property', 'featuremap_property',
      'study_property', 'nd_experiment_types', 'nd_geolocation_property',
    ...
    $n_transferred = 0;
    $n_removed = 0;
    foreach ($obsolete_cvs as $obsolete_cv) {
      $query = $chado->select('1:cv', 'cv');
      $query->condition('name', $obsolete_cv, '=');
      $query->fields('cv', ['cv_id']);
      $cv_id = $query->execute()->fetchField();

      // Only update if the CV exists
      if ($cv_id) {
        ...
        if ($count) {
          $this->output()->writeln(t('Transferred @count records from CV "@from" to the "local" CV',
              ['@count' => $count, '@from' => $obsolete_cv]));
          $n_transferred += $count;
        }
        $query = $chado->delete('1:cv');
        $query->condition('name', $obsolete_cv, '=');
        $query->execute();
        $n_removed++;
      }
    ...
    if (!$n_transferred and !$n_removed) {
      $this->output()->writeln(t('No changes were needed'));
    }
  }
