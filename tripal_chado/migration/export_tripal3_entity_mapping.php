 ...
 *   The columns are: chado table name, chado table pkey column, chado table pkey value, entity id value
 */
function find_all_published(string $output_file_name) {
  ...
  // only go as far as getting the IDs of published records
  $grand_total = 0;
  ...
  foreach($bundle_names as $bundle_name => $bundle_label) {
    ...
    if ($results) {
      while ($record = $results->fetchAssoc()) {
        fwrite($outf, implode("\t", [$table, $pkey_field, $record[$pkey_field], $record['entity_id']]) . "\n");
        $total++;
        $grand_total++;
      }
    }
