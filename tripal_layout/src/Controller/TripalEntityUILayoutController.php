...
class TripalEntityUILayoutController extends ControllerBase {
  ...
  public function applyLayout($tripal_entity_type) {
    ...
    $this->hideComponent('title', $display);
    $this->hideComponent('uid', $display);

    $settings = ['label' => $bundle_label . ' Details', 'open' => TRUE];
    $this->addDetailsFieldGroup('summary', $display, $settings);

    $settings = ['label' => $bundle_label . ' Table', 'parent_name' => 'summary', 'open' => TRUE];
    $this->addTableFieldGroup('summary_table', $display, $settings);
    $this->setChild('organism_comment', 'summary_table', $display);
