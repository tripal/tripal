...
abstract class TripalWidgetBase extends WidgetBase {
   ...
   * we don't want to display 100 fields for every delta if they are not populated.
   * See Drupal issue https://www.drupal.org/project/drupal/issues/1156338
   * We will tell Drupal that it is unlimited cardinality up until the point where
   * we actually hit the cardinality limit. Before that point, we can use the
   * "Add another item" button in the same way as unlimited cardinality fields.
   *
   * @inheritdoc
   */
  protected function formMultipleElements(FieldItemListInterface $items, array &$form, FormStateInterface $form_state) {
    ...
    // Have the parent class generate the elements with the adjusted cardinality.
    ...
    // This will only happen after the ajax triggers from a previous row addition.
    if ($cardinality > 1) {
