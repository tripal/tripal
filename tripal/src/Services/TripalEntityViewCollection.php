...
class TripalEntityViewCollection implements ContainerInjectionInterface  {
  ...
  /**
   * Instantiates a new TripalEntityViewCollection object.
   */
  public function __construct(
    TripalLogger $config_factory,
    TripalLogger $logger,
  ) {
 ...
 #   $config_factory = \Drupal::service('config.factory');
    $config_list = $this->config_factory->listAll('tripal.tripalentityview_collection');

    $collections = [];
    foreach ($config_list as $config_item) {
      $config = $this->config_factory->get($config_item);
   ...
   *
   * @param array $view_ids
   *   An array of the config id's of views you would like to
   *   install, e.g. ['views.view.chado_analysis'].
   * @param bool $reload
   *   If the view already exists and this is TRUE, then
   *   reload the view from configuration.
   */
  public function install(array $view_ids, bool $reload = FALSE) {
#    $yaml_prefix = 'tripal.tripalentityview_collection.';
...
#      $config_item = $yaml_prefix . $config_id;
      $config = $$this->config_factory->get($view_id);

      // If view already exists and we want to reload, delete existing view.
      if (is_object($config) && $reload) {
        // @todo
        $config = NULL;
      }
      ...
      if (!is_object($config)) {
        ...
        $this->logger->notice("Creating view \"$view_id\"");
