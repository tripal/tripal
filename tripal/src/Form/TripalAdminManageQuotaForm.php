...
class TripalAdminManageQuotaForm implements FormInterface{
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    try {
      // Set user quota.
      if (!\tripal_set_user_quota($user->id(), $size, $expiration)) {
        $this->messenger()
          ->addError(t('Unable to set quota. A database error occurred'));
      }
    ...
    return new RedirectResponse(Url::fromRoute('tripal.files_quota')->toUriString());
  }
