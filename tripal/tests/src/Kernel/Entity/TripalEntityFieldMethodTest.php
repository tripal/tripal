...
class TripalEntityFieldMethodTest extends TripalTestKernelBase {
   ...
   * @var string
   */
  protected string $yaml_info_file = __DIR__ . '/TripalEntityFieldMethodTest-TestInfo.yml';
  ...
  public function testTripalEntityFieldMethods(int $current_scenario_key, string $current_scenario_label) {
    ...
    // -- required fields for drupal_sql_storage should be detected.
    $required_tripal_fields = $created_entity->getTripalStorageFields(
      'drupal_sql_storage',
      ['is_required' => TRUE]
    );
    $this->assertEquals($current_scenario['expectations']['getStorageFields']['required_fields'], $required_tripal_fields, "We expected to have detected the required tripal fields for our " . $current_scenario['label'] . " scenario.");

    // -- optional fields for drupal_sql_storage should be detected.
