...
class TripalFieldValueLookupTest extends ChadoTestKernelBase {
   ...
   */
  #[DataProvider('provideDataForGetUniqueFieldValues')]
  public function testGetUniqueFieldValues(string $scenario, string $field_name, string $property, array $filters, array $options, array $expected_values) {
    ...
    $values = $lookup->getUniqueFieldValues($field_name, $property, ['bundles' => $bundles], []);

    // Check that we got the expected values.
    foreach ($expected_values as $expected) {
      $this->assertContains($expected, $values, "The expected value" . $expected[$field_name . '_value'] . " was not found in the results in the case of " . $scenario . ".");
    }
  }
