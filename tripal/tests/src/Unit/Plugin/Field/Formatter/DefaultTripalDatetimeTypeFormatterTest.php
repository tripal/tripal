...
class DefaultTripalDatetimeTypeFormatterTest extends UnitTestCase {
  ...
  /**
   * Tests the output from several predefined timestamp formats.
   *
  ...
  /**
   * Special values for hiding the field.
   *
   * These are used for testViewElementsHidesSuppressedValue().
   */
  ...
  /**
   * Both '-infinity' and '0' are always hidden regardless of hide_condition.
   *
  ...
  public function testSettingsSummaryDefaultFormat(): void {
    ...
    $entry = $summary[0];
    $this->assertStringContainsString('Y-m-d H:i:s', $entry);
    $this->assertStringContainsString('2025-01-15 10:30:00', $entry);
  }
  ...
  public function testSettingsSummaryCustomFormat(): void {
    ...
    $this->assertStringContainsString('F j, Y', $entry);
    $this->assertStringContainsString('January 15, 2025', $entry);
  }

  /**
   * Empty date_format falls back to default fromat.
   */
  public function testSettingsSummaryEmptyFormatFallback(): void {
    $this->formatter->setSetting('date_format', '');
    $summary = $this->formatter->settingsSummary();
    $entry = $summary[0];
    $this->assertStringContainsString('Y-m-d H:i:s', $entry);
  }
